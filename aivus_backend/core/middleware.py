...
class HMACAuthenticationMiddleware(MiddlewareMixin):
    ...
    def _get_user_context(self, user, user_group):
        """Get vendor/client IDs for user."""
        vendor_id = None
        client_id = None
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if api_key == settings.API_KEY and user_id and user_group:
            try:
                user = User.objects.get(id=user_id)
                request.user_data = self._get_user_context(user, user_group)
                return None
